...
struct DownloadingContent {
    ...
    var resumeData: Data?
    var activity : Activity<FlixyLiveActivityAttributes>?
...
class DownloadViewModel: BaseViewModel, URLSessionDownloadDelegate {
    ...
    var timer = Timer()
    var activities = Activity<FlixyLiveActivityAttributes>.activities
...
////        }
    ...
    func createActivity( contentDownload: DownloadContent?) {
        ...
        let contentState = FlixyLiveActivityAttributes.LiveDeliveryData(status: LiveActivityStatus.pending.title, progress: 0.0)
        do {
            let newActivity = try Activity<FlixyLiveActivityAttributes>.request(
                attributes: FlixyLiveActivityAttributes(
                    imageUrl: id,
                    downloadId: id,
    ...
    func update(activity: Activity<FlixyLiveActivityAttributes>, progress: Double) {
        Task {
            let contentState = FlixyLiveActivityAttributes.LiveDeliveryData(status: "Downloading...", progress: progress)
            await activity.update(using: contentState)
        }
    }
    
    func updateStatus(activity: Activity<FlixyLiveActivityAttributes>, status: String, downloadContentId: String) {
        Task {
            let contentState = FlixyLiveActivityAttributes.LiveDeliveryData(status: status, progress: Double(downloadingContents[downloadContentId]?.progress ?? 0))
            await activity.update(using: contentState)
        }
    }
    
    func end(activity: Activity<FlixyLiveActivityAttributes>) {
        Task {
            let contentState = FlixyLiveActivityAttributes.LiveDeliveryData(status: "Download completed", progress: 1.0)
