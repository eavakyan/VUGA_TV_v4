            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    if vm.isLoading && vm.movies.isEmpty {
                        ...
                        // Movies Grid
                        LazyVGrid(columns: columns, spacing: 15) {
                            ForEach(vm.movies, id: \.id) { content in
                                MovieGridItem(content: content)
                            }
                        }
                        .padding(.horizontal)
...
class MoviesListViewModel: BaseViewModel {
    @Published var movies: [VugaContent] = []
    ...
    func fetchMovies() {
        ...
        let params: [Params: Any] = [
            .type: ContentType.movie.rawValue,
            .page: currentPage
        ]
        ...
        NetworkManager.callWebService(url: .fetchContents, params: params) { (obj: ContentsModel) in
            ...
            if let contents = obj.data {
                if self.currentPage == 1 {
                    ...
                    self.movies.append(contentsOf: contents)
                }
    ...
    func sortContent(by option: String) {
        switch option {
        case "Latest":
            movies.sort { ($0.createdAt ?? "") > ($1.createdAt ?? "") }
        case "Popular":
            movies.sort { ($0.totalView ?? 0) > ($1.totalView ?? 0) }
        case "Top Rated":
            movies.sort { ($0.ratings ?? 0) > ($1.ratings ?? 0) }
        case "A-Z":
