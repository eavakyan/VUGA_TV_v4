...
class ActorController extends Controller
{
    ...
    public function fetchActorDetails(Request $request)
    {
        ...
        $contents = $actor->contents()
            ->where('is_show', 1)
            ->with(['language', 'genres', 'ageLimits'])  // FIXED: Removed 'contentCast.actor' which doesn't exist
        ...
        $actorData['actorContent'] = $contents->map(function($content) use ($actor) {
            $data = [
                ...
                'type' => $content->type,
                'duration' => (string)$content->duration,  // Convert to string for iOS compatibility
